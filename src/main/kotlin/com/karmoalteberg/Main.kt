...
fun generateOutput(roDatabase: ReadDatabase, csvFilesDirectoryPath: String): List<Request> {
	...
	val dateTransformer = DateTransformer(DateTransformer.GLOBAL_DATE_FORMAT, true)
	...
	val csvFiles = File(csvFilesDirectoryPath).walk().filter { it.name.matches(Regex("input_[0-9]+\\.csv")) }.toList()
